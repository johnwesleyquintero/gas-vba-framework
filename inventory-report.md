...
<body>
    ...
    <div class="form-group">
        ...
        <input type="text" id="contextInput" placeholder="e.g., 'Q2 Inventory Review', 'Pre-Prime Day Analysis'">
    </div>

    <div class="btn-group">
        <button class="btn" id="generateAllButton" onclick="generateDashboard('all')">Generate All Listings Dashboard</button>
        <button class="btn" id="generateInactiveButton" onclick="generateDashboard('inactive')">Generate Inactive Listings (FBA-SKU) Dashboard</button>
        <button class="btn" id="generateActiveButton" onclick="generateDashboard('active')">Generate Active Listings (Sorted by DoS) Dashboard</button>
    </div>
    ...
    <script>
        function showStatus(message, type = 'error') {
            ...
            // Use clearTimeout if you want to prevent multiple timeouts overlapping
            setTimeout(() => { statusAlert.style.display = 'none'; }, 5000); // Hide after 5 seconds
        }

        function setButtonsState(disabled) {
            document.getElementById('generateAllButton').disabled = disabled;
            document.getElementById('generateInactiveButton').disabled = disabled;
            document.getElementById('generateActiveButton').disabled = disabled;
        }

        function generateDashboard(reportType) {
            const context = document.getElementById('contextInput').value;
            const loadingSpinner = document.getElementById('loadingSpinner');

            // Disable all buttons, show loading spinner
            setButtonsState(true);
            loadingSpinner.style.display = 'block';
            document.getElementById('statusAlert').style.display = 'none'; // Clear previous status
            ...
            google.script.run
                ...
                .createConsolidatedDashboardFromSidebar(context, reportType);
        }
    </script>
</body>
</html>
